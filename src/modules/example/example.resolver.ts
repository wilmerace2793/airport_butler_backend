...
export class ExampleResolver {
  ...
  @Query(() => [Example], { name: 'examples' }) // plural
  findAll() {
    return this.exampleService.findAll();
  }

  @Query(() => Example, { name: 'example' }) // singular
  ...
  @Mutation(() => Example)
  updateExample(
    @Args('updateExampleInput') updateExampleInput: UpdateExampleInput,
  ) {
    return this.exampleService.update(
      updateExampleInput.id,
      updateExampleInput,
    );
  }
